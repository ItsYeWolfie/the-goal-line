...
export default function FixturesMain() {
	...
	useEffect(() => {
		const fetchAPI = async () => {
			const data = await fetchData<IFixture[]>(currentUrl);
			...
			{loading ? (
				...
				matches.map((match, index: number) => (
					<Link
						...
						} relative grid grid-cols-3 items-center rounded-xl py-3  md:grid-cols-12  `}
					>
						<FixtureMainInfo
							match={match}
							hoverdMatchId={hoveredMatchId}
						/>
						<div className="col-span-1 justify-self-end text-center text-sm md:col-span-3 md:text-lg">
						...
						<p className="col-span-1 mx-auto w-max rounded-lg bg-sky-700 px-4 text-center text-sm text-white md:col-span-2 md:text-lg">
							{match.goals.home}:{match.goals.away}
						</p>
						<div className="col-span-1 justify-start text-center text-sm md:col-span-3 md:text-lg">
							<LogoAndImage
						...
						<p className="col-span-1 hidden font-medium text-sky-600 dark:text-sky-400 md:col-span-2 md:grid  md:text-center md:text-lg">
							Time: {moment(match.fixture.date).format('HH:MM')}
						</p>
						<FaInfoCircle
							className="col-span-1 hidden w-full text-right text-sky-600 dark:text-sky-400 md:col-span-1 md:grid md:text-lg"
