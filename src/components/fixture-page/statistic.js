...
class Statistic extends LitLightElement {
		...
		render() {
			...
			return html`
			<div class="mb-4 flex h-auto w-full flex-col justify-evenly rounded-md bg-gray-800 pt-4 pb-4 md:ml-20 md:w-4/5 lg:ml-40 lg:w-full">
				${this.statistic.statistics.map((team) => html`
					${team.statistics.map((statistic) => html`
						<span class="flex justify-evenly">
							<span>${statistic.value}</span>
							<div class="block w-[68%] text-center lg:w-[50%]">${statistic.type}</div>
							</span>
							<span class="flex justify-center">
								<span class="flex h-2 w-44 justify-end rounded-md bg-slate-300 md:w-72 lg:w-72">
									<span class="w-${statistic.value} rounded-md bg-lime-700"></span>
								</span>
								<span class="ml-1 flex h-2 w-44 rounded-md bg-slate-300 md:w-72 lg:w-72">
